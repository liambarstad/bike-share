...
require './app/models/weather_date'
...
Ccsv.foreach('db/csv/weather.csv') do |row|
  if header == false && row[0].to_i < 6000
    ...
    weather = Weather.new(date_id: WeatherDate.find_or_create_by(date: row[0]),
                              ...
                              zip_code_id: ZipCode.find_or_create_by(zip_code: row[23]))
      weather.save

      puts "Adding weather #{count} to the table"
      count += 1
